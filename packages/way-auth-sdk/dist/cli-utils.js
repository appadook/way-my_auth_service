...
export function normalizePublicPaths(value) {
    if (!value) {
        return undefined;
    }
    const parsed = value
...
function buildMiddlewareConfigBlock(options) {
    const entries = [];
...
export function buildNextAuthFile(options = {}) {
...
export function buildNextMiddlewareFile() {
    return [
...
export function buildNextEnvUpdates(baseUrl) {
...
export function buildNextProxyRewrites(baseUrl) {
    const normalizedBaseUrl = normalizeBaseUrl(baseUrl);
    return [
        {
...
export function buildNextRewritesSnippet(baseUrl) {
    const rewrites = buildNextProxyRewrites(baseUrl);
    return [
        "async rewrites() {",
        "  return [",
        ...rewrites.map((rewrite) => `    { source: "${rewrite.source}", destination: "${rewrite.destination}" },`),
        "  ];",
        "},",
    ].join("\n");
}
function hasWayAuthProxyRewrites(content) {
    return (content.includes("/.well-known/way-auth-configuration") &&
        content.includes("/api/v1/:path*"));
}
function appendPropertyBlockToObjectBody(objectBody, propertyBlock) {
...
export function patchNextConfigWithProxyRewrites(existingContent, baseUrl) {
...
export function buildSetupGuide(config) {
...
export function buildNextSetupGuide(input) {
    const transportModeNote = input.transportMode === "proxy"
